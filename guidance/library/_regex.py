...
def tree_to_grammar(node, nots):
    ...
    if not node.sons:
        val = node.head.value
        return '' if val in ['Epsilon', 'Empty'] else string(val)
    ...
    if node.head.value == 'Concatenation':
        return ''.join(tree_to_grammar(x, nots) for x in node.sons)
...
def merge_nodes(regex, op):
    ...
    if len(current.sons) and current.head.value == op:
        ...
        if (
            all(len(x.sons) == 0 for x in current.sons)
            and current.head.value == op
        ):
