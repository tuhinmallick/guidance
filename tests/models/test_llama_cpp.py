...
def test_llama_cpp_gen():
    ...
    lm = f"{lm}this is a test" + gen("test", max_tokens=10)
    assert len(str(lm)) > len("this is a test")

def test_llama_cpp_gen_log_probs():
    lm = get_model("llama_cpp:")
    lm = f"{lm}this is a test" + gen("test", max_tokens=1)
...
def test_llama_cpp_recursion_error():
    ...
    lm = f"""{lm}Tweak this proverb to apply to model instructions instead.
...
def test_repeat_calls():
    ...
    lm = f'{llama2}How much is 2 + 2? ' + gen(name='test', max_tokens=10)
    a = [lm['test']]
    lm = f'{llama2}How much is 2 + 2? ' + gen(
        name='test', max_tokens=10, pattern=r'\d+'
    )
    a.append(lm['test'])
    lm = f'{llama2}How much is 2 + 2? ' + gen(name='test', max_tokens=10)
    a.append(lm['test'])
...
def test_suffix():
    llama2 = get_model("llama_cpp:")
    lm = f'{llama2}1. Here is a sentence ' + gen(
        name='bla', list_append=True, suffix='\n'
    )
    assert (str(lm))[-1] == '\n'
    assert (str(lm))[-2] != '\n'
...
def test_subtoken_forced():
    llama2 = get_model("llama_cpp:")
    lm = f'{llama2}How much is 2 + 2? ' + gen(
        name='test', max_tokens=10, regex=r'\('
    )
