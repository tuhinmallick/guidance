...
def test_fstring_custom():
    ...
    @guidance
    def my_function(lm):
        return f'{lm}another {select(["item1", "item2"])}'

    lm += f'this is a test {my_function()}'
    assert str(lm) in {
        "this is a test another item1",
        "this is a test another item2",
    }

def test_token_count():
    lm = get_model("transformers:gpt2")
    lm2 = f'{lm} 1 1 1 1 1{gen(max_tokens=9)}{gen(max_tokens=9)}'
...
def test_token_healing():
    ...
    lm = f"{gpt2}This is a story of 10 or 5 or {zero_or_more(byte_range(b'0', b'9'))}"
