...
def test_one_or_more():
    ...
    assert parser.valid_next_bytes() == {Byte(b'a')}
...
def test_zero_or_more_and_one_or_more():
    ...
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {Byte(b'a'), Byte(b'b')}
    parser.consume_byte(b'a')
    assert parser.valid_next_bytes() == {Byte(b'a'), Byte(b'b')}
    parser.consume_byte(b'b')
    assert parser.valid_next_bytes() == {Byte(b'b')}
...
def test_zero_or_more_and_one_or_more_mixed():
    ...
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {Byte(b'a'), Byte(b't')}
    parser.consume_byte(b't')
    parser.consume_byte(b'e')
    parser.consume_byte(b's')
    assert parser.valid_next_bytes() == {Byte(b't')}
    parser.consume_byte(b't')
    assert parser.valid_next_bytes() == {Byte(b'b')}
...
def test_select():
    ...
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {Byte(b'b'), Byte(b's')}
    parser.consume_byte(b's')
    assert parser.valid_next_bytes() == {Byte(b'u')}
    parser.consume_byte(b'u')
    assert parser.valid_next_bytes() == {Byte(b'e')}

def test_select_nested():
    g = select(["bob", "bill", select(["mark", "mary"])])
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {Byte(b'b'), Byte(b'm')}
    parser.consume_byte(b'm')
    assert parser.valid_next_bytes() == {Byte(b'a')}
    parser.consume_byte(b'a')
    assert parser.valid_next_bytes() == {Byte(b'r')}
    parser.consume_byte(b'r')
    assert parser.valid_next_bytes() == {Byte(b'k'), Byte(b'y')}
...
def test_select_joined():
    ...
    assert parser.valid_next_bytes() == {Byte(b'b')}
    parser.consume_byte(b'b')
    assert parser.valid_next_bytes() == {Byte(b'o'), Byte(b'i')}
    parser.consume_byte(b'i')
    assert parser.valid_next_bytes() == {Byte(b'l')}
    parser.consume_byte(b'l')
    assert parser.valid_next_bytes() == {Byte(b'l')}
    parser.consume_byte(b'l')
    assert parser.valid_next_bytes() == {Byte(b'm')}
    parser.consume_byte(b'm')
    assert parser.valid_next_bytes() == {Byte(b'a')}
    parser.consume_byte(b'a')
    assert parser.valid_next_bytes() == {Byte(b'r')}
...
def test_char_set():
    g = char_set("b-f")
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {ByteRange(b'bf')}
    parser.consume_byte(b'b')
...
def test_string_utf8():
    ...
    g = string("¶")
    parser = EarleyCommitParser(g)
    assert parser.valid_next_bytes() == {Byte(b[:1])}
